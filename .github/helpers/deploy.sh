...
# its desk dependencies with peru (see peru.yaml — a curated subset of base-dev)
# and ship the finished desk down to the ship VM. The remote only unpacks it and
# commits — it needs no peru, no git, and no urbit clone. This replaces rsyncing
# ALL of pkg/base-dev onto the desk (the new kernel builds every mark present,
# and many unused base-dev marks don't compile on the target kelvin).
